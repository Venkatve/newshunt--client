...
function EditNews() {
  ...
  const save = async () => {
    ...
    try {
      ...
      await axios.post("https://news-hunt-app.herokuapp.com/api/newsitems/editnewsitem", payload);
      setLoading(false);
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const result = await axios.post("https://news-hunt-app.herokuapp.com/api/newsitems/getnewsitembyid", {
        newsid: params.newsid,
      });
