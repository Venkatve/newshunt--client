...
function PostedNewsItems() {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const result = await axios.post("https://news-hunt-app.herokuapp.com/api/newsitems/getnewsitemsbyuserid", {
        userid: user._id,
      });
      setLoading(false);
  ...
  const deleteItem = async (newsid) => {
    setLoading(true);
    try {
      await axios.post("https://news-hunt-app.herokuapp.com/api/newsitems/deletenewsitem", {
        newsid,
      });
      getData()
    } catch (error) {
